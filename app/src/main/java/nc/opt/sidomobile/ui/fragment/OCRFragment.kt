...
import nc.opt.sidomobile.ocr.OCRGraphic
import java.io.IOException
import java.util.*
import java.util.concurrent.Semaphore
import java.util.concurrent.TimeUnit

class OCRFragment : androidx.fragment.app.Fragment(), ActivityCompat.OnRequestPermissionsResultCallback {
    /**
    ...
    private val onSuccessListener = { firebaseVisionText: FirebaseVisionText ->
        ...
        for (a: FirebaseVisionText.TextBlock in firebaseVisionText.textBlocks) {
            val graphic = OCRGraphic(mGraphicOverlay)
            graphic.textBlock = a
     ...
     * still image is ready to be read.
     */
    private val mOnImageAvailableListener = { reader: ImageReader ->
        val image = reader.acquireLatestImage()
        if (!detectorIsRunning) {
            Log.d(TAG, "Taille de l'image = " + image?.width + " x " + image?.height)
            Log.d(TAG, "Taille du graphicOverlay = " + mGraphicOverlay?.width + " x " + mGraphicOverlay?.height)
    ...
    private fun setUpCameraOutputs(width: Int, height: Int) {
        ...
        try {
            ...
            for (cameraId in manager.cameraIdList) {
                ...
                Log.d(
                    TAG,
                    "Taille du graphicOverlay en début de programme " + mGraphicOverlay!!.width + " x " + mGraphicOverlay!!.height
                )
                mImageReader = ImageReader.newInstance(
                    mGraphicOverlay!!.width,
                    mGraphicOverlay!!.height,
    ...
    private fun configureTransform(viewWidth: Int, viewHeight: Int) {
        if (null == mTextureView || null == mPreviewSize || null == appCompatActivity) {
            return
        }
